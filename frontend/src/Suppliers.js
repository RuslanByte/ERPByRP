...
function Suppliers({ handleCombinedClick, gstyleTog, bstyleTog, onClickAdd, addstyleTog, linestyleTog }){
  ...
  const fetchAllData = useCallback( async () => {
    try {
      const response = await fetch(`/api/suppliers/limit?numOfRow=${debouncedNumOfRow}`, {
        method: 'GET',
        headers: {
  ...
  const handleSearch = async (event) => {
    if(event.key === 'Enter'){
      ...
      try {
        const response = await fetch(`/api/suppliers/search?searchTerm=${encodeURIComponent(searchTerm)}`,{
          method: 'GET',
          headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
  ...
  const handleSave = async (editData) => {
    ...
    if(editData){
      try {
        const response = await fetch(`/api/suppliers/${encodeURIComponent(editData.id)}/updateSupplier`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
    ...
    return <div className = "gnavR">
        ...
        <ul className = "rlu">
            ...
            <img src = {A} alt = "d" width = "18px" height = "18px" vspace = "2" hspace = "2" align ="left"/> Добавить нового поставщика
            </li>
        ...
        <div className = "info">Информация о поставщиках</div>
        <div>
      ...
      <table>
        ...
        <tbody className="dataProductsStyleT">
          {data.map((item, index) => (
            <tr key={index}>
              <td>{editData && editData.id === item.id  ? ( <input className='editInput' type='text' name='city' value={editData.name} onChange={handleInputChange}/>) : (item.name)}</td>
